    ...
    <section className="py-10 md:py-14 bg-white">
      <div className="mx-auto max-w-[clamp(320px,90vw,1280px)] px-4 md:px-8">
        <div className="flex items-center justify-between mb-6">
          ...
          <Link href="/news" className="text-sm text-slate-700 hover:underline">{t("seeAll")}</Link>
        </div>
        <div className="grid md:grid-cols-3 gap-4 md:gap-6">
          {items.map(x => (
            <Link key={x.id} href={x.href} className="rounded-2xl border border-slate-200 p-5 hover:shadow-sm">
