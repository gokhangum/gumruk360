...
async function makeSupabase() {
  ...
  return createServerClient(url, key, {
    cookies: {
  get: async (name) => (await cookies()).get(name)?.value, // ← SONA VİRGÜL
...
} // ← cookies bloğunu kapatan } eklendi
}); // ← createServerClient(...) çağrısını kapatır
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const { error: uerr } = await supabase.from("pricing_versions").update({ is_active: true }).eq("id", version_id);
    if (uerr) throw uerr;
    ...
    await supabase.from("audit_logs").insert({
      ...
      actor_role: "admin",
      action: "update",
      event: "pricing.version_activated",
