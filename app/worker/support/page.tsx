...
export default async function SupportListPage() {
  ...
  const t = await getTranslations('worker.support')
  ...
  async function deleteTicketAction(formData: FormData) {
    ...
    // Sahiplik kontrolü
    const { data: ticketRow } = await a.from("contact_tickets").select("id,user_id").eq("id", id).single()
    if (!ticketRow || ticketRow.user_id !== myId) throw new Error(t('errors.unauthorized'))
  ...
  async function markResolvedAction(formData: FormData) {
    ...
    if (ticketRow.question_id) {
      const { data: me } = await a.from("profiles").select("email,full_name").eq("id", myId).single()
      const senderEmail = me?.email || ""
      const senderName = me?.full_name || senderEmail || t('mail.userFallbackName')
      const qLink = `${base}/admin/request/${ticketRow.question_id}?email=${encodeURIComponent(senderEmail)}`
      ...
      const fallback = process.env.ADMIN_EMAIL || process.env.SUPPORT_EMAIL || ""
      if (!toList.length && fallback) toList = [fallback]

      if (toList.length && resend) {
        const subject = t('mail.resolvedSubject')
        const html = `
          <div style="font-family:Arial,Helvetica,sans-serif;line-height:1.5;font-size:14px">
           <p><strong>${t('mail.labels.questionId')}:</strong> <a href="${qLink}" target="_blank" rel="noopener noreferrer">${ticketRow.question_id}</a></p>
<p><strong>${t('mail.labels.reporter')}:</strong> ${senderName} &lt;${senderEmail}&gt;</p>
<p>${t('mail.resolvedInfo')}</p>
          </div>
        `
  ...
  <div className="px-3 md:px-5 pt-2 md:pt-3 pb-3 md:pb-5">
    <div className="card-surface shadow-colored rounded-xl">
        ...
        <Link href="/worker/contact" className="btn btn--primary btn--cta text-xs md:text-sm h-9 px-3 whitespace-nowrap">
          {t('newTicket')}
        </Link>
      </div>
    </div>
      ...
      <div className="md:hidden">
         {rows.length ? (
          rows.map((row) => (
            <div key={row.id} className="border-b px-3 py-3">
                   <div className="flex items-center gap-2 text-[11px] text-slate-500">
           <span>{t('table.date')}</span>
               <span className="font-mono text-slate-700">{new Date(row.created_at).toLocaleString()}</span>
              </div>
              <div className="mt-1">
                <div className="text-[11px] text-slate-500">{t('table.subject')}</div>
               <div className="text-sm font-medium">{row.subject || "—"}</div>
              </div>
             <div className="mt-1">
                 <div className="text-[11px] text-slate-500">{t('table.question')}</div>
                <div className="text-sm">
                   {row.question_id ? (
                     <Link className="text-blue-700 underline break-all" href={`/dashboard/questions/${row.question_id}`}>
                     {row.question_id}
                     </Link>
                  ) : "—"}
                </div>
             ...
             <span className="text-[11px] text-slate-500">{t('table.status')}</span>
...
: row.status === "answered"
              ...
              <div className="mt-2 flex flex-wrap gap-2">
               <Link className="btn btn--ghost text-xs md:text-sm h-8 px-3" href={`/worker/support/${row.id}`}>
                  {t('action.open')}
                </Link>
                {/* Çözüldü */}
               <form action={markResolvedAction}>
                  <input type="hidden" name="id" value={row.id} />
                 <button className="btn btn--ghost text-xs md:text-sm h-8 px-3" type="submit">
               ...
               {/* Sil */}
                <form action={deleteTicketAction}>
                  <input type="hidden" name="id" value={row.id} />
                  <button className="btn btn--danger text-xs md:text-sm h-8 px-3" type="submit">
                    {t('action.delete')}
                   </button>
                 </form>
             </div>
        ...
        <table className="min-w-full text-sm">
           <thead className="bg-gray-50">
            <tr className="text-left">
             ...
             <th className="px-3 py-2">{t('table.question')}</th>
            ...
            {rows.map((row) => (
               ...
               <td className="px-3 py-2">
                 <div className="flex flex-wrap gap-2">
                   <Link className="btn btn--ghost text-sm" href={`/worker/support/${row.id}`}>
                     {t('action.open')}
                    </Link>
                    {/* Çözüldü */}
                   <form action={markResolvedAction}>
