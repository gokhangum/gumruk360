...
import FilePicker from "./FilePicker"; // worker tarafında AttachmentsPicker yerine FilePicker kullanıyoruz
import MessageBodySmart from "../components/MessageBodySmart";
import { getMessages, getTranslations } from "next-intl/server";
import { MAIL } from "../../../../lib/config/appEnv";

/* ========= i18n yardımcıları ========= */
// Dot-notation ile güvenli mesaj erişimi
...
async function getTSafe(ns: string) {
  try {
    ...
    return ((key: string, values?: Record<string, any>) => {
      let raw = dotGet(nsObj, key);
	  if (raw === undefined) raw = msgs?.[`${ns}.${key}`];
      if (typeof raw === "string") {
        return raw.replace(/\{(\w+)\}/g, (_m, v) => (values && v in values ? String(values[v]) : `{${v}}`));
      }
   try {
   // Nested/flat yakalanamazsa Next-Intl’e tam anahtar ile bırak
    return baseT(`${ns}.${key}`, values as any);
  } catch {}
      return `${ns}.${key}`;
...
export async function replyTicketAction(formData: FormData) {
  "use server";
  const t = await getTSafe("worker.support"); // worker namespace
  ...
  // Mesaj ekle (role fallback customer -> user)
  async function insertMsg(role: "customer" | "user") {
    return a
      .from("contact_messages")
      .insert({ ticket_id: ticketId, sender_role: role, body: messageText })
      .select("id")
      .single();
  }
  ...
  // Admin e-posta bildirimi (worker sayfasından cevaplandı)
  ...
  try {
    ...
    const workerTicketUrl = `${base}/worker/support/${ticketId}`;
    ...
    const envAdminEmails = (process.env.ADMIN_EMAILS || "")
      ...
      .map((s) => s.trim())
      .filter(Boolean);
    const finalAdminEmails = Array.from(new Set([...adminEmails, ...envAdminEmails]));
    ...
    if (finalAdminEmails.length && resend) {
      const html = `
        <div style="font-family:Arial,Helvetica,sans-serif;line-height:1.5;font-size:14px">
          ...
          <div style="margin-top:12px">
            ...
            &nbsp;|&nbsp;
            <a href="${workerTicketUrl}" target="_blank" rel="noopener noreferrer">${t("adminEmail.userReply.linkWorker")}</a>
          </div>
        </div>
      `;
      ...
      await a.from("notification_logs").insert({
        ...
        payload: {
          ticket_id: ticketId,
          worker_ticket_url: workerTicketUrl,
  ...
  revalidatePath(`/worker/support/${ticketId}`);
  redirect(`/worker/support/${ticketId}?sent=1`);
}

export async function markResolvedFromDetailAction(formData: FormData) {
  "use server";
  const t = await getTSafe("worker.support");
  const s = await supabaseServer();
  const a = admin();
  ...
  try {
    ...
    const envAdminEmails2 = (process.env.ADMIN_EMAILS || "")
      ...
      .map((s) => s.trim())
      .filter(Boolean);
    toList = Array.from(new Set([...toList, ...envAdminEmails2]));
...
export default async function SupportDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: u } = await s.auth.getUser();
  if (!u?.user) redirect(`/login?next=/worker/support/${id}`);
  const userId = u.user.id;
  ...
  // Başlık önceliği: soru başlığı varsa onu kullan
  ...
  if (ticket?.question_id) {
    const { data: q } = await a
      ...
      .maybeSingle();
    if (q?.title) displayTitle = q.title;
  }
  ...
  // Ekleri DB’den al + storage ile eşle
  const { data: attachRows } = await a
    .from("contact_attachments")
    .select("object_path,file_name,size,uploaded_by,created_at")
    .eq("ticket_id", id);
  ...
  const contactFiles: Array<{ id: string; name: string; openUrl: string; uploadedAt?: string; by?: "admin" | "me" }> = [];
  if (!listResp.error && Array.isArray(listResp.data)) {
    for (const f of listResp.data) {
      ...
      const sizeFromList = (f as any)?.metadata?.size ?? (f as any)?.size ?? 0;
      ...
      if (!pushedIds.has(id2)) {
        ...
        const rec = (attachRows || []).find(r => r.object_path === fullPath);
        const by: "admin" | "me" | undefined = rec ? (rec.uploaded_by === userId ? "me" : "admin") : undefined;
        const uploadedAt = rec?.created_at ? new Date(rec.created_at).toLocaleString() : undefined;
        contactFiles.push({ id: id2, name: displayName, openUrl, uploadedAt, by });
      }
    }
  }
        ...
        <div className="card-surface shadow-colored rounded-xl">
          ...
          <div className="p-3 overflow-x-auto">
            <div className="card-surface p-4">
              ...
              <div>
                <span className="font-medium">{t("detail.labels.status")}:</span>{" "}
                {ticket.status === "open"
                  ? t("status.open")
              ...
              <div>
                ...
                {ticket.question_id ? (
                  <a className="text-blue-700 underline" href={`/worker/questions/${ticket.question_id}`}>
                    {ticket.question_id}
                  </a>
                ) : "—"}
              </div>
                  ...
                  return (
                    <div key={m.id} className="card-surface p-3 md:p-4">
                      <div className="text-[11px] text-gray-500 mb-1 flex items-center gap-2">
                        ...
                        {isLast && (
                          <form action={markResolvedFromDetailAction} className="inline">
                            ...
                            <button type="submit" className="ml-2 btn btn--ghost text-xs" title={t("ui.markResolved.title")}>
                      ...
                      <div className="whitespace-pre-wrap text-sm">
                        <MessageBodySmart text={m.body ?? ""} />
                      </div>
                    </div>
                  );
                })}
                {(!msgs || msgs.length === 0) && (
                  <div className="text-gray-500 text-sm">{t("detail.noMessages")}</div>
                )}
              </div>
            </div>
          </div>
                  ...
                  >
                    ...
                    {f.uploadedAt && (
                      <div className="text-[11px] text-gray-500 mt-0.5">
                        {f.by === "me" ? t("detail.by.me") : t("detail.by.admin")} • {f.uploadedAt}
                      </div>
                    )}
                  </a>
                ))}
          ...
          <div className="card-surface p-4 space-y-2 edge-underline edge-blue edge-taper edge-rise-2mm mt-3 md:mt-3">
            ...
            <form action={replyTicketAction} className="space-y-3">
              ...
              <div>
                ...
                <textarea
                  name="message"
                  className="w-full rounded px-3 py-2 min-h-[120px] bg-white/90 ring-1 ring-slate-200 focus:ring-2 focus:ring-blue-300 outline-none"
                  placeholder={t("ui.reply.placeholder")}
                  required
                />
              </div>
              <div>
                <label className="block text-sm mb-1">{t("ui.filePicker.title")}</label>
                {/* Worker tarafında FilePicker kullanılıyor */}
                <FilePicker />
              </div>
