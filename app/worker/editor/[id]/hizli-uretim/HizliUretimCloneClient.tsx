...
export default function HizliUretimCloneClient(props: {
  ...
  const t = useTranslations("worker.editor.fast.ui");
  ...
  function setTempText(idx: number, value: string){
    setTempTexts(prev => prev.map((txt,i)=> i===idx ? value : txt))
  }
  useEffect(()=>{
  ...
  const exportMarkdown = useMemo(()=>{
    const lines: string[] = []
    if (resText?.trim()) {
      lines.push('## ' + t('markdown.resultTitle'), resText.trim())
    }
    if (Array.isArray(sources) && sources.length>0){
      lines.push('', '## ' + t('markdown.sourcesTitle'))
  ...
  async function runInternal(targetLang: 'tr'|'en', englishHint: boolean){
    ...
    try{
      const r = await fetch('/api/worker/gpt-answers/run' + buildQsWithLang(targetLang), {
        method: 'POST',
      ...
      if (!j?.ok){
        ...
        alert(j?.error || t('errors.runFailed'))
        setLoading(false)
        return
      }
      ...
      alert(t('errors.runException', { detail: e?.message || String(e) }))
    } finally {
      setLoading(false)
    }
  }
  ...
  async function summarize(){
    setSumLoading(true)
    try{
      const r = await fetch('/api/worker/gpt-answers/summarize' + buildQsWithLang(), {
        method:'POST',
      ...
      if(j?.ok){ setResText(j.data?.text || '') }
        ...
        alert(j?.error || t('errors.sumFailed'))
      }
    } catch(e:any){
      
      alert(t('errors.sumException', { detail: e?.message || String(e) }))
  ...
  function goToEditor(){
    ...
    window.open(`/worker/editor/${props.questionId}${qs}`, '_blank', 'noopener,noreferrer')
  }
    ...
    <div className="grid md:grid-cols-2 gap-4">
      ...
      <div className="p-4 border rounded-md">
        <div className="grid gap-2 mb-2">
          <label className="flex items-center gap-2"><span className="w-28">{t('styleLabel')}</span>
            <select value={style} onChange={(e)=>setStyle(e.target.value as 'teknik'|'resmi')} className="border rounded px-2 py-1">
               <option value="teknik">{t('styleOption.teknik')}</option><option value="resmi">{t('styleOption.resmi')}</option>
            </select>
          </label>
        </div>

        <div className="mb-2">
          <div className="text-xs text-gray-600 mb-1">{t('questionLabel')}</div>
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <div className="text-xs text-gray-600">{t('extraTexts.label')}</div>
             <button onClick={()=>setTempTexts(v=>[...v, ''])} className="text-xs border rounded px-2 py-1">{t('extraTexts.add')}</button>
          </div>
          {tempTexts.map((txt, i)=>(
            <div key={i} className="flex items-start gap-2">
              <textarea className="w-full border rounded p-2 h-16" value={txt} onChange={e=>setTempText(i, e.target.value)} />
              <button onClick={()=>setTempTexts(v=> v.filter((_,idx)=> idx!==i))} className="text-xs border rounded px-2 py-1">{t('extraTexts.remove')}</button>
            </div>
          ))}
        </div>
        ...
        <div className="space-y-2 mt-3">
          <div className="flex items-center justify-between">
            <div className="text-xs text-gray-600">{t('files.label')}</div>
              ...
              {tempFiles.map((f, i)=>(
                <li key={i} className="p-2 flex items-center justify-between gap-2">
                  <div className="min-w-0">
                    ...
                    <div className="text-[11px] text-gray-500">{prettySize(f.size)} · {f.type || t('files.typeFallback')}</div>
                  </div>
                  <button onClick={()=>setTempFiles(v=> v.filter((_,idx)=> idx!==i))} className="text-xs border rounded px-2 py-1">{t('files.remove')}</button>
                </li>
              ))}
            </ul>
      ...
      <div className="border rounded p-3">
        <div className="text-xs text-gray-600 mb-1">{t('resultLabel')}</div>
        <textarea className="w-full border rounded p-2 h-72" value={resText} onChange={e=>setResText(e.target.value)} />

        <h4 className="font-semibold mt-4">{t('sourcesLabel')}</h4>
