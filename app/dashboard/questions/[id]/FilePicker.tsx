...
buttonText,
  ...
  multiple = true,
	...
	const t = useTranslations("questions.filePicker")
  const rid = useId();
  ...
  return (
    <div>
      <label className="block text-sm mb-1">{labelText ?? t("label")}</label>
      <input
        ref={inputRef}
        type="file"
        id={id}
      ...
      >
        ...
        <span>{buttonText ?? t("choose")}</span>
      </label>

      {/* Selected files list */}
      <div className="mt-2">
        {files.length === 0 ? (
          <div className="text-xs text-gray-500" id={helpId}>
            {/* no files chosen */}
          </div>
        ) : (
          <div className="text-xs">
            <div className="flex items-center justify-between mb-1">
              <span className="text-gray-600">{t("selected", { count: files.length })}</span>
